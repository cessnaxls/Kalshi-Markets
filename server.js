...
const EXEC_BASE=ENV==='production'?'https://external-api.kalshi.com/trade-api/v2':'https://external-api.demo.kalshi.co/trade-api/v2';
...
async function kalshi(p,{auth=false,method='GET',body,data=false}={}){const base=data?DATA_BASE:(auth?EXEC_BASE:DATA_BASE);const r=await fetch(base+p,{method,headers:auth?authHeaders(method,p):{'Content-Type':'application/json'},body:body?JSON.stringify(body):undefined}); const text=await r.text(); let d;try{d=JSON.parse(text)}catch{d={raw:text}} if(!r.ok)throw Object.assign(new Error(d?.message||d?.error?.message||`Kalshi ${r.status}`),{status:r.status,data:d}); return d;}
...
app.get('/api/account',async(q,r)=>{try{r.json({balance:await kalshi('/portfolio/balance',{auth:true}),positions:await kalshi('/portfolio/positions?limit=1000',{auth:true}),orders:await kalshi('/portfolio/orders?limit=1000',{auth:true}),environment:ENV})}catch(e){r.status(e.status||500).json({error:e.message})}});
app.get('/api/account/summary',async(q,r)=>{try{const [balance,positions]=await Promise.all([kalshi('/portfolio/balance',{auth:true}),kalshi('/portfolio/positions?limit=1000&count_filter=position',{auth:true})]);r.json({balance,positions,environment:ENV})}catch(e){r.status(e.status||500).json({error:e.message})}});
