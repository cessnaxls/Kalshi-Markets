...
function execEnv(){return runtimeEnv||ENV} function execBase(){return execEnv()==='production'?'https://external-api.kalshi.com/trade-api/v2':'https://external-api.demo.kalshi.co/trade-api/v2'};
const DATA_ENV=process.env.KALSHI_DATA_ENV==='demo'?'demo':'production';
const DATA_BASE=DATA_ENV==='demo'?'https://external-api.demo.kalshi.co/trade-api/v2':'https://external-api.kalshi.com/trade-api/v2';
let keyId=process.env.KALSHI_API_KEY_ID||''; let privateKey=(process.env.KALSHI_PRIVATE_KEY||'').replace(/\\n/g,'\n'); let runtimeEnv=null;
...
async function kalshi(p,{auth=false,method='GET',body,data=false}={}){const base=data?DATA_BASE:(auth?execBase():DATA_BASE);const r=await fetch(base+p,{method,headers:auth?authHeaders(method,p):{'Content-Type':'application/json'},body:body?JSON.stringify(body):undefined}); const text=await r.text(); let d;try{d=JSON.parse(text)}catch{d={raw:text}} if(!r.ok)throw Object.assign(new Error(d?.message||d?.error?.message||`Kalshi ${r.status}`),{status:r.status,data:d}); return d;}
async function chartData(ticker,days=6){
  const md=await kalshi('/markets/'+encodeURIComponent(ticker),{data:true}); const m=md.market;
  let ev={}; try{ev=(await kalshi('/events/'+encodeURIComponent(m.event_ticker),{data:true})).event||{}}catch{}
  const end=Math.floor(Date.now()/1000); days=Math.max(1,Math.min(+days||6,6)); const start=end-days*86400;
  let c;
  try{
    const q=new URLSearchParams({market_tickers:ticker,start_ts:String(start),end_ts:String(end),period_interval:'1',include_latest_before_start:'true'});
    const batch=await kalshi('/markets/candlesticks?'+q,{data:true});
    c={ticker,candlesticks:batch.markets?.[0]?.candlesticks||[]};
  }catch(firstErr){
    // Trade-derived fallback keeps the chart usable when the candle service rejects a window.
    const q=new URLSearchParams({ticker,limit:'1000',min_ts:String(start),max_ts:String(end)});
    const td=await kalshi('/markets/trades?'+q,{data:true});
    const buckets=new Map();
    for(const tr of td.trades||[]){const ts=Math.floor(new Date(tr.created_time).getTime()/1000);if(!Number.isFinite(ts))continue;const t=Math.floor(ts/60)*60;const px=+tr.yes_price_dollars;if(!Number.isFinite(px))continue;const vol=+(tr.count_fp||0);let b=buckets.get(t);if(!b)b={end_period_ts:t,price:{open_dollars:String(px),high_dollars:String(px),low_dollars:String(px),close_dollars:String(px)},volume_fp:String(vol)};else{b.price.high_dollars=String(Math.max(+b.price.high_dollars,px));b.price.low_dollars=String(Math.min(+b.price.low_dollars,px));b.price.close_dollars=String(px);b.volume_fp=String(+b.volume_fp+vol)}buckets.set(t,b)}
    c={ticker,candlesticks:[...buckets.values()].sort((a,b)=>a.end_period_ts-b.end_period_ts),fallback:'trades',candle_error:firstErr.message};
    if(!c.candlesticks.length)throw firstErr;
  }
  return {...c,market:m,event:ev,series_ticker:ev.series_ticker||'',data_environment:DATA_ENV};
}
...
app.get('/api/account',async(q,r)=>{try{r.json({balance:await kalshi('/portfolio/balance',{auth:true}),positions:await kalshi('/portfolio/positions?limit=1000',{auth:true}),orders:await kalshi('/portfolio/orders?limit=1000',{auth:true}),environment:execEnv()})}catch(e){r.status(e.status||500).json({error:e.message})}});
app.get('/api/account/summary',async(q,r)=>{try{const [balance,positions]=await Promise.all([kalshi('/portfolio/balance',{auth:true}),kalshi('/portfolio/positions?limit=1000&count_filter=position',{auth:true})]);r.json({balance,positions,environment:execEnv()})}catch(e){r.status(e.status||500).json({error:e.message})}});
...
app.listen(process.env.PORT||3000,()=>console.log(`Kalshi Edge Terminal v9 (${ENV})`));
